        ...
        public void onClick(View v) {
            ...
            if (mItem.isBus()) {
                if (mHolder.arrowExpend == false) {
                    ...
                    BusStationItem departure = mItem.getBusLine().getDepartureBusStation();
                    departure.setBusStationName("始发站 " + departure.getBusStationName());
                    addBusStation(mItem.getBusLine().getDepartureBusStation());
                    for (BusStationItem station : mItem.getBusLine()
                            .getPassStations()) {
                        station.setBusStationName("经过 " + station.getBusStationName());
                        addBusStation(station);
                    }
                    BusStationItem arrival = mItem.getBusLine().getArrivalBusStation();
                    arrival.setBusStationName("终点站 " + arrival.getBusStationName());
                    addBusStation(arrival);
                ...
                if (mHolder.arrowExpend == false) {
                    ...
                    mHolder.busExpandImage
                            .setImageResource(R.drawable.up);
                    RailwayStationItem departure = mItem.getRailway().getDeparturestop();
                    departure.setName("始发站 " + departure.getName());
                    addRailwayStation(departure);
                    for (RailwayStationItem station : mItem.getRailway().getViastops()) {
                        station.setName("经过 " + station.getName());
                        addRailwayStation(station);
                    }
                    RailwayStationItem arrival = mItem.getRailway().getArrivalstop();
                    arrival.setName("终点站 " + arrival.getName());
                    addRailwayStation(arrival);
